...
USE_PRETRAINED = True  # Change to False if you want to train from scratch

class MobileNetV2(nn.Module):
    def __init__(self):
        super(MobileNetV2, self).__init__()
        self.model = models.mobilenet_v2(pretrained=USE_PRETRAINED)
        self.model.classifier[1] = nn.Linear(self.model.last_channel, 1)  # Modify output layer
...
class GoogleNet(nn.Module):
    def __init__(self, aux_logits=False):
        ...
        self.model.fc = nn.Linear(self.model.fc.in_features, 1)  # Modify output layer
        self.sigmoid = nn.Sigmoid()
...
class AlexNet(nn.Module):
    def __init__(self):
        ...
        self.model.classifier[6] = nn.Linear(self.model.classifier[6].in_features, 1)  # Modify output layer
        self.sigmoid = nn.Sigmoid()
